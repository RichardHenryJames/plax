    ...
    <nav className="fixed top-0 left-0 right-0 z-50 gradient-top pointer-events-none">
      <div className="flex items-center justify-between px-5 py-4 pointer-events-auto">
        ...
        <div className="flex items-center gap-1">
          ...
          <Link href="/profile" className="p-2 text-dark-muted hover:text-white transition-colors rounded-full hover:bg-white/5">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {!loading && (
            <div className="relative">
              {user ? (
                ...
                <button
                  onClick={() => setShowMenu(!showMenu)}
                  className="px-3 py-1.5 bg-gradient-to-r from-violet-600 to-cyan-600 rounded-lg text-white text-xs font-semibold hover:shadow-lg hover:shadow-violet-500/20 transition ml-1"
                >
                  Sign in
                </button>
              )}
                      ...
                      {user ? (
                        <>
                          ...
                          <Link
                            href="/profile"
                            onClick={() => setShowMenu(false)}
                            className="flex items-center gap-2 px-3 py-2.5 text-sm text-dark-text hover:bg-white/5 transition"
                          >
                            <span>👤</span> Profile & Stats
                          </Link>
                          <button
                            onClick={() => { signOut(); setShowMenu(false) }}
                            className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-red-400 hover:bg-white/5 transition"
                          >
                            <span>🚪</span> Sign Out
                          </button>
                        </>
                        ...
                        <>
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-dark-text hover:bg-white/5 transition"
                          >
