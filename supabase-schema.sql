...
DROP POLICY IF EXISTS "Users can view own engagements" ON public.engagements;
CREATE POLICY "Users can view own engagements" ON public.engagements
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own engagements" ON public.engagements;
